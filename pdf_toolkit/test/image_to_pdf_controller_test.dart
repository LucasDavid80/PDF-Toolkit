...
void main() {
  ...
  group('ImageToPdfController - Testes Unitários Positivos', () {
    test(
      ...
      () async {
        final pickResult = FilePickerResult([
          PlatformFile(
            name: 'image1.png',
            path: 'C:\\path\\image1.png',
            size: 100,
          ),
          PlatformFile(
            name: 'image2.jpg',
            path: 'C:\\path\\image2.jpg',
            size: 200,
          ),
          PlatformFile(
            name: 'image3.jpeg',
            path: 'C:\\path\\image3.jpeg',
            size: 300,
          ),
        ]);
    ...
    test(
      '4. Converter com lista válida + caminho de saída válido -> sucesso',
      () async {
        ...
        when(
          () => mockPdfCombiner.createPDFFromMultipleImages(
  ...
  group('ImageToPdfController - Testes Unitários Negativos', () {
    test(
      '1. Tentar adicionar arquivo com extensão não suportada (.gif) -> rejeitado, lista não muda, erro reportado',
      () async {
        controller.images.value = ['C:\\path\\img1.png'];
        final pickResult = FilePickerResult([
          PlatformFile(
            name: 'invalid.gif',
            path: 'C:\\path\\invalid.gif',
            size: 100,
          ),
          PlatformFile(
            name: 'valid.jpg',
            path: 'C:\\path\\valid.jpg',
            size: 200,
          ),
        ]);
    ...
    test(
      ...
      () async {
        ...
        expect(
          controller.errorMessage.value,
          contains('Nenhuma imagem selecionada'),
        );
        verifyNever(
          () => mockFilePicker.saveFile(
    ...
    test(
      '3. PdfCombiner lança exceção (imagem corrompida) -> captura e expõe mensagem amigável',
      () async {
        ...
        expect(
          controller.errorMessage.value,
          contains(
            'Uma ou mais imagens selecionadas parecem corrompidas ou inválidas.',
          ),
        );
    ...
    test(
      ...
      () async {
        ...
        expect(
          controller.errorMessage.value,
          contains(
            'Não foi possível salvar o arquivo em "C:\\output\\result.pdf". Verifique se possui permissões de escrita.',
          ),
        );
        expect(controller.successMessage.value, isNull);
        expect(controller.isProcessing.value, isFalse);
