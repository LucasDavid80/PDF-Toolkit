...
void main() {
  ...
  group('MergePdfController - Testes Unitários Positivos', () {
    test(
      '1. Adicionar PDFs válidos -> lista atualizada corretamente',
      () async {
    ...
    test(
      '4. Unir com 2+ PDFs válidos -> PdfCombiner.mergeMultiplePDFs chamado corretamente, retorno de sucesso',
      () async {
        ...
        when(
          () => mockPdfCombiner.mergeMultiplePDFs(
  ...
  group('MergePdfController - Testes Unitários Negativos', () {
    test(
      '1. Tentar adicionar arquivo com extensão não suportada -> rejeitado, lista não muda, erro reportado',
      () async {
        controller.pdfs.value = ['C:\\path\\doc1.pdf'];
        final pickResult = FilePickerResult([
          PlatformFile(
            name: 'invalid.txt',
            path: 'C:\\path\\invalid.txt',
            size: 50,
          ),
          PlatformFile(
            name: 'valid.pdf',
            path: 'C:\\path\\valid.pdf',
            size: 150,
          ),
        ]);
        ...
        expect(
          ...
          contains('Alguns arquivos foram ignorados'),
        );
      },
    );

    test(
      '2. Tentar unir com apenas 1 PDF selecionado -> ação bloqueada, erro exposto',
      () async {
        controller.pdfs.value = ['C:\\path\\doc1.pdf'];
        ...
        expect(
          controller.errorMessage.value,
          contains('Selecione ao menos 2 arquivos PDF para unir.'),
        );
        verifyNever(
          () => mockFilePicker.saveFile(
    ...
    test(
      '3. PdfCombiner lança exceção (PDF corrompido/protegido) -> captura e expõe mensagem amigável',
      () async {
        controller.pdfs.value = [
          'C:\\path\\doc1.pdf',
          'C:\\path\\corrupted.pdf',
        ];
        const outputPath = 'C:\\output\\merged.pdf';
        ...
        expect(
          controller.errorMessage.value,
          contains(
            'Um ou mais arquivos PDF parecem corrompidos ou protegidos por senha.',
          ),
        );
    ...
    test(
      '4. Falha de escrita simulada (permissão negada) -> expõe erro de I/O claro',
      () async {
        controller.pdfs.value = ['C:\\path\\doc1.pdf', 'C:\\path\\doc2.pdf'];
        const outputPath = 'C:\\output\\merged.pdf';
        ...
        expect(
          controller.errorMessage.value,
          contains(
            'Não foi possível salvar o arquivo em "C:\\output\\merged.pdf". Verifique se possui permissões de escrita.',
          ),
        );
        expect(controller.successMessage.value, isNull);
        expect(controller.isProcessing.value, isFalse);
    ...
    test(
      '5. Cancelar diálogo de salvar -> interrompe sem erro e sem processamento',
      () async {
        controller.pdfs.value = ['C:\\path\\doc1.pdf', 'C:\\path\\doc2.pdf'];
